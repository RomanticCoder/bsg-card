...
const Product: NextPage<PageProps> = (props) => {
  ...
  const onInputChange :ChangeEventHandler<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>= (e) => {
    dispatch({
      name: e.target.name,
  ...
  const onFileChange:React.ChangeEventHandler<HTMLInputElement & { files: FileList }> = (event) => {
    // const {
    //   target: { files },
    // } = event;
  ...
  const storeLocally = (cartItemObj: TypeCartItem) => {
    let cartItems: any[] = [];
  ...
  const storeOnFirebase = async (cartItemObj:TypeCartItem) => {
  ...
  const onSubmit:React.FormEventHandler = (e) => {
    ...
    const cartItemObj:TypeCartItem = {
      price: item.price,
          ...
          >
            ...
            {item?.options &&
              item.options?.map((option) => {
                ...
                console.log(option)
                const name:string = Object.keys(option)[0];
                    ...
                    <div className="inline-block relative w-64">
                      ...
                      >
                        ...
                        {values  && values?.map((value:string) => (
                          <option key={`option_${name}_${value}`} value={value}>
                            {value}
                          </option>
                        ))}
                      </select>
